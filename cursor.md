...
- `src/components/ui/OrbitalCard.tsx` — floating card with drift animation + staging lerp; **`forwardRef`** + **`tick(now)`** + **`getVideoElement()`** via **`useImperativeHandle`** (**Session 87**, **Session 90**). Position: **`transform: translate(x - 110, y - 80) translateZ(0)`** from fixed `0,0` (**Session 90** + **Session 101** — GPU layer on **`cardRef`**). Listens to **`portfolio:orbit-staging`** for dock and **`portfolio:chat-response-complete`** for release delay (**Session 93**).
- `src/components/ui/OrbitalSystem.tsx` — mounts 10 cards, computes home positions + **Session 93** dock targets (measured panel + **`DOCK_GAP = CARD_W / 2`**, same-side vertical stagger); **`z-10`**; **one `requestAnimationFrame` loop** invokes each card's **`tick`** only when **`pathname === '/'`** (**Session 87**, **Session 90**). Listens to **`portfolio:project-active`**, batches, emits **`portfolio:orbit-staging`** (**Session 93**). **`activeVideoRef`** + **`onVideoHover` / `onVideoLeave`** — one orbital MP4 at a time (**Session 90**; **Session 99** — deferred pause, **80ms** leave debounce, no **`currentTime`** reset on leave; **Session 102** — no **`video.style.opacity`**; see Performance).
...
- **Session 90 — `transform` positioning:** `cardRef.current.style.transform = translate(x - 110px, y - 80px) translateZ(0)` with **`left: 0` / `top: 0` / `position: fixed`** — no per-frame `left`/`top` updates. **`willChange: 'transform'`** on outer wrapper.
- **Session 101 — Orbital `cardRef` GPU layer:** **`translateZ(0)`** on initial style **and** every **`tick`** transform write — prevents compositor **demotion** on hover-off (which caused full-site jank with Swirl). No CSS **`transform`** on **`.orbital-card-panel`** / **`.orbital-card-root`**; card position transform is **JS-only**.
...
- **Session 99 — Orbital video hover-off:** Never **`pause()`** synchronously on pointer events — schedule with **`requestAnimationFrame`**. **`leaveTimerRef`** + **`80ms`** delay before pausing after **`onVideoLeave`**; cancel timer on re-hover (same card before pause: early return, no **`currentTime`** seek — playback continues). **`currentTime = 0`** only when **entering** a card (or switching cards), not on leave — avoids seek/decode jank. Previous card on switch: **`pause()`** in **`requestAnimationFrame`** only (no seek). Cleanup clears **`leaveTimerRef`** on unmount.
- **Session 102 — Orbital video (no `video.style.opacity`):** Do not toggle **`video.style.opacity`** in **`OrbitalSystem`** — use **`pause()`** only on leave (after **80ms**) and when switching cards. Video element stays fully opaque; last frame remains visible when paused.
- **Session 100 — Orbital panels (no backdrop blur):** **`backgroundColor: rgba(14,16,21,0.88)`** on **`absolute` / `inset: 0` / `zIndex: 0`** — **no** **`backdrop-filter`** (Swirl behind the cards is animated; backdrop sampling caused hover-off jank). **`opacity`** idle/hover/active on **`opacityLayerRef`**. **`cardRef`**: **`transform`** (includes **`translateZ(0)`** per **Session 101**) + **`zIndex`** + **`willChange: 'transform'`** (static). Beacon + **`.orbital-card-panel`** (**`transparent`**). Nav, **`ChatPanel`**, sticky chrome blurs **unchanged** — rule is **orbital cards only**.
...
**Do NOT change** staging lerp factor **`0.06`**, traffic light colors, or orbital chrome layout. (**Session 100:** solid fill on **`absolute` inset-0** layer — **no** **`backdrop-filter`** on orbital cards. **Session 94:** no hover **scale** on the panel.)
...
**Session 99 — orbital video:** Do not **`pause()`** or set **`currentTime`** synchronously on pointer enter/leave. Do not reset **`currentTime`** on leave — only on hover enter (or when switching videos). Keep **80ms** leave debounce and **`requestAnimationFrame`**-deferred **`pause()`** in **`OrbitalSystem`**.
**Session 102 — orbital video:** No **`video.style.opacity`** anywhere in orbital card / **`OrbitalSystem`** code.
**Session 100 — orbital cards vs Swirl:** No **`backdrop-filter`** on orbital card surfaces. Use solid **`rgba(14,16,21,0.88)`** on the inset-0 layer. **`willChange: 'transform'`** only on the **`cardRef` wrapper, always on** — never toggle; **`globals.css`** must not add **`will-change`** / **`transform`** to **`.orbital-card-panel`** for hover (border/shadow/opacity only). No hover **`scale`**. Nav / **`ChatPanel`** / page chrome blurs stay as-is.
**Session 101 — orbital `cardRef` layer promotion:** **`translateZ(0)`** in the **`cardRef`** transform string on mount **and** every physics **`tick`** — pairs with **`will-change: transform`** so layers are not torn down on hover-off (fixes Swirl-wide freeze). Do not strip **`translateZ(0)`** from transform writes.
...
- **Session 90:** That RAF runs **only when `pathname === '/'`** — off homepage the loop does not schedule (no physics ticks per frame). **`activeVideoRef`** + **`leaveTimerRef`** (**Session 99**) coordinate orbital MP4 playback.
