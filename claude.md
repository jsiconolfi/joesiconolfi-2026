...
- **Contextual cards (Session 72, Session 74, **Session 76**, **Session 78**, **Session 96**, **Session 97**, **Session 105**):** `Message` may include optional `cards?: ChatCard[]` (label, description, href, type — **`description` not rendered** after Session 76; still present in API/`INITIAL_MESSAGE` for meta). **Session 96 / Session 97:** **`SYSTEM_PROMPT`** includes a **`CRITICAL CARD RULE`** block **immediately before** "Surfacing contextual cards" (waypoint = design system / components / Sherpa; waypoint-sync = CLI / token sync / Figma-to-code / design-map.json / pipeline). Surfacing rules: design system / components / Sherpa → **`waypoint`** + **`sherpa`**; token sync / Figma-to-code / waypoint-sync / token pipelines → **`waypoint-sync` only**, never **`waypoint`**. **Session 105:** Case study keys **`channel-nexus`** and **`channel-prism`**; surfacing: Nexus / multi-model comparison → **`channel-nexus`**; Prism / consumer image generation → **`channel-prism`**; general Channel AI work → **`channel`**, **`channel-nexus`**, **`channel-prism`** (max 3 cards). **`CARD_META`** includes **`channel-nexus`** → `/work/channel-nexus`, **`channel-prism`** → `/work/channel-prism`. Server validates trailing JSON with **`.filter((c) => c in CARD_META)`** (exact keys only). **`CARD_META['waypoint-sync']`** → `/work/waypoint-sync`. **Session 78:** Cards are **per message only** — no `useState` for cards. Greeting is committed with **`cloneGreetingMessage()`** (copy of `INITIAL_MESSAGE` + shallow-copied `cards` array) so state never holds the module singleton. New assistant rows use **`cards: undefined`** until a **`{ type: 'cards' }`** NDJSON line applies **`setMessages`** for that **`assistantMessage.id`** only; completion sets **`isStreaming: false`** without overwriting **`cards`** (no local `cards` accumulator merged at end). Render with **`message.cards && message.cards.length > 0 && !message.isStreaming`**. Rendered only below **completed** assistant text (not while `isStreaming`). **`role="button"`** row: single line label + `→`; padding `8px 12px`; list wrapper `gap: 6`, `marginTop: 10`. **Session 76:** Internal routes use **`useRouter().push(card.href)`** so **Framer Motion** page transitions run; **`mailto`** and **`.pdf`** use **`window.open(..., '_blank')`**. No `<a href>` for internal navigation. **Session 74:** static border/bg; **text-only hover** — `.card-title` and `.card-arrow` → `#00ff9f`; at rest title `rgba(255,255,255,0.7)` (weight 300), arrow `rgba(0,255,159,0.5)`; `Enter` activates focused row. Separate from suggestion chips.
...
- `src/components/ui/OrbitalCard.tsx` — floating card with sine-wave drift + staging lerp; **`forwardRef`** + **`useImperativeHandle`** exposes **`tick(now)`** + **`getVideoElement()`** (**Session 87**, **Session 90** video coordinator). Position written as **`transform: translate(...) translateZ(0)`** (**Session 90** + **Session 101** GPU layer — see Performance). Listens to **`portfolio:orbit-staging`** for dock; **`portfolio:chat-response-complete`** + **`POST_STREAM_RELEASE_MS`** for return timing (**Session 93**).
- `src/components/ui/OrbitalSystem.tsx` — mounts 10 cards, computes home positions + **Session 93** staging targets (dock math + multi-mention stagger); **single shared `requestAnimationFrame` loop** calls each card's **`tick`** when **`pathname === '/'`** (**Session 87**, **Session 90** pause off homepage). **`activeVideoRef`** enforces one playing MP4 (**Session 90**; **Session 99** — **`leaveTimerRef`**, **80ms** delayed pause, **`requestAnimationFrame`** pause only, **`currentTime`** reset on hover enter only; **Session 102** — never **`video.style.opacity`**; paused video stays visible as last frame). **`src/lib/orbitalStaging.ts`** — **`ORBIT_STAGING_EVENT`**, **`CHAT_RESPONSE_COMPLETE_EVENT`**, **`OrbitStagingDetail`**.
...
- Card body (**Session 100** — supersedes Session 92 frosted glass): **no `backdrop-filter`** on orbital cards (avoids sampling the animated Swirl canvas every frame). **`position: absolute; inset: 0; zIndex: 0`** fill layer: **`backgroundColor: rgba(14, 16, 21, 0.88)`** only (**`pointer-events: none`**), **sibling** to the content stack. Idle/hover/active **`opacity`** lives on **`opacityLayerRef`** (`relative` **`zIndex: 1`**), not **`cardRef`**. **`cardRef`**: **`transform`** (includes **`translateZ(0)`** — **Session 101**), **`borderRadius: 8px`**, **`zIndex`**, **`willChange: 'transform'`** (permanent, never toggled). **`.orbital-card-panel`** **`transparent`**. Nav, chat, sticky chrome blurs unchanged. `220px` wide
...
- Hover: border → `rgba(255,255,255,0.2)` (**Session 94:** no `scale`; **Session 100:** no `transform` / `will-change` changes on `.orbital-card-panel` — only border/shadow), `zIndex: 12` — implemented in **`globals.css`** via `.orbital-card-root:hover` (no `hovered` React state — **Session 73**)
...
- `project.video` (and MP4 in `project.image` when no separate still) → `<video muted playsInline loop preload="metadata">` — loads enough for a **first-frame thumbnail at rest**; **`preload="none"`** leaves orbital cards visually empty until hover. Plays on hover or chat activation; pauses on leave without seeking (**Session 99** — coordinator resets **`currentTime`** only on **enter** / card switch, not on leave).
...
**Viewport clamping (Session 23, **Session 97**):**
- `clampHome(xPct, yPct, vw, vh)` in `OrbitalSystem.tsx` — ensures every home position keeps the full 220×160 card footprint on screen. **Session 97:** **`HOME_MARGIN` = 16px** (matches per-frame clamp in **`OrbitalCard`**). Re-applied on every resize.
...
- **Session 96:** Per-frame **edge repulsion removed** — old `EDGE_MARGIN` / `EDGE_FORCE` velocity nudges were tuned for snappier springs and caused oscillation near viewport edges with Session 94 slow physics. Do not reintroduce edge velocity forces without an explicit session.
- **Session 97 — Hard position clamp (not a force):** After spring/damping/drift/staging integration each frame, **`posRef`** (**card center**) is clamped so the full **220×160** footprint stays inside the viewport with **`VIEWPORT_MARGIN` = 16px** on all edges (`min/max` use **center** coords: `MARGIN + HALF_W` / `vw - MARGIN - HALF_W`, etc.). No bounce — position is capped; **velocity on that axis is zeroed** when a boundary is hit so energy does not accumulate. **`clampHome`** nudges initial homes; this clamp is the authoritative bound every frame (fixes the old bug where center was clamped as if it were top-left).
...
- **Session 97** viewport clamp is mandatory every frame — not optional "last resort" padding
...
**Do NOT change** staging lerp factor **`0.06`**, traffic light colors, or orbital chrome layout. (**Session 100:** orbital cards use **solid** **`rgba(14,16,21,0.88)`** fill — **no** **`backdrop-filter`**. **Session 94:** hover uses border brightening only — **no** `scale` on **`.orbital-card-panel`**.)
...
**Performance — Session 30 + Session 73 + Session 87 + Session 90 + Session 99 + Session 100 + Session 101 + Session 102 (homepage / orbital):**
- **Session 90 — Position via `transform: translate(x, y)`** — outer wrapper is `position: fixed; left: 0; top: 0` with **`transform: translate(x - 110px, y - 80px) translateZ(0)`** (`110`/`80` = half of `220×160` footprint; **`translateZ(0)`** from **Session 101**). Physics `x,y` remain card-center coordinates; compositor-friendly (no per-frame `left`/`top` layout). **`willChange: 'transform'`** on the wrapper only.
- **Session 101 — Permanent GPU layer on orbital `cardRef`:** Browsers can demote compositor layers hinted with **`will-change: transform`** when they consider them inactive; hover-off on cards caused a **0.5–1s** full-site stall (Swirl/nav freeze). **`translateZ(0)`** appended to **every** transform string (initial inline style **and** each **`tick`** write) keeps a **3D transform** active so layers stay promoted from mount to unmount. **`globals.css`** must not set **`transform`** on **`.orbital-card-panel`** / **`.orbital-card-root`** — positioning is **JS-only** on the outer wrapper.
...
- **Session 90 — Single orbital video** — `OrbitalSystem` holds **`activeVideoRef`**; **`onVideoHover(index)`** / **`onVideoLeave(index)`** coordinate playback. **`OrbitalCardHandle`** adds **`getVideoElement()`** for the coordinator. Prevents stacked MP4 decodes on fast hover sweeps.
- **Session 99 — Orbital video hover-off perf** — Do not **`pause()`** or seek on the synchronous pointer path. **`handleOrbitalVideoLeave`**: **`80ms`** **`setTimeout`** then null **`activeVideoRef`**, then **`requestAnimationFrame` → `pause()`** (no **`currentTime`** on leave). **`handleOrbitalVideoHover`**: cancel pending leave timer; if same **`HTMLVideoElement`** as **`activeVideoRef`** (re-enter within **80ms**), return without seek/play. Otherwise **`requestAnimationFrame` → `pause()`** on the previous video only (no seek); set **`currentTime = 0`** and **`play()`** on the newly hovered video (**`readyState` / `canplay`** unchanged). Cleanup **`leaveTimerRef`** on unmount.
- **Session 102 — Orbital video: no element opacity hacks** — Do not assign **`video.style.opacity`** in **`OrbitalSystem`** (enter, leave, or card switch). Hiding the element on leave looked wrong; **`pause()`** after the **80ms** delay keeps the last frame visible. Switching cards: **`pause()`** the previous video in **`requestAnimationFrame`** only — no opacity.
- **Session 100 — Orbital cards vs Swirl perf:** **No `backdrop-filter`** on orbital card surfaces — it forced the compositor to sample the **60fps Swirl canvas** behind the cards; hover-off layer changes caused visible jank. Replaced with **opaque-tinted** **`rgba(14,16,21,0.88)`** on the **`absolute` inset-0** layer (no live backdrop). **`willChange: 'transform'`** stays **only** on the **outer `cardRef` wrapper** (permanent, never toggled); **`globals.css`** does **not** set **`will-change`** or **`transform`** on **`.orbital-card-panel`** (hover changes **border** / **box-shadow** / **opacity** timings **`0.1s`** only). **`opacityLayerRef`** transition **`opacity 0.1s`**. Nav, `ChatPanel`, overlays, sticky `*.exe` headers, `TabBar` — **keep** their blurs; rule applies to **orbital cards only**.
- **Session 94 — Ambient drift + hover perf:** Idle spring **`SPRING = 0.004`**, **`DAMPING = 0.96`** in **`OrbitalCard`**; **`DRIFT_CONFIGS`** in **`OrbitalSystem`** — smaller amplitudes (**8–14px**), **~0.0003** speeds, **`phase = index * (2π/10)`**. Hover border only on **`.orbital-card-panel`** (**no** **`scale`**).
...
- **Session 73 — hover visuals are CSS-only** — classes `orbital-card-root`, `orbital-card-panel`, `orbital-card-panel--active`, footer helpers in **`globals.css`**. Moving the cursor across cards does **not** re-render React for border, shadow, role color, or footer copy swap. **Session 100:** compositor **`will-change: transform`** on **`cardRef`** wrapper only — **not** on **`.orbital-card-panel`**; no hover **scale**. **Session 101:** **`translateZ(0)`** on the same **`cardRef`** transform stack (not CSS).
...
- **OrbitalCard video** — play/pause coordinated by **`OrbitalSystem`** (**Session 90**, **Session 99** — see **`leaveTimerRef`** / deferred pause rules above). **`preload="metadata"`** on `<video>` (**Session 74**) — required so MP4-backed cards show a poster at rest. **`OrbitalCard`** does not call **`pause()`** directly; **`onVideoHover` / `onVideoLeave`** only.
...
**Session 99 — Orbital video:** Do not **`pause()`** or reset **`currentTime`** synchronously on **`mouseenter` / `mouseleave`**. Do not reset **`currentTime`** on leave — only on hover enter (or when switching to another card’s video). Keep **80ms** leave delay and **`requestAnimationFrame`**-deferred **`pause()`** in **`OrbitalSystem`** unless an explicit session changes it.
**Session 102 — Orbital video:** Never set **`video.style.opacity`** in **`OrbitalSystem`** or orbital card code — video stays visible whenever the card is visible; **`pause()`** only.
**Do NOT change:** staging lerp factor (`0.06`), traffic light colors, card visual design (terminal chrome, **Session 94:** no hover grow — border-only hover). **Do NOT add `backdrop-filter`** to orbital cards (**Session 100**).
**Do NOT reintroduce:** elliptical orbital math, `next/image` in OrbitalCard (intentional exception), pairwise **card-card collision** repulsion (**Session 95** removed), per-frame **edge velocity repulsion** (**Session 96** removed `EDGE_MARGIN`/`EDGE_FORCE`), or **per-card `requestAnimationFrame`** in `OrbitalCard` (**Session 87** — one RAF in `OrbitalSystem` calls **`tick(now)`** on each card)
**Do NOT remove:** `willChange: 'transform'` + **`translateZ(0)`** in the **`cardRef`** transform string on OrbitalCard (**Session 90** position + **Session 101** layer promotion — not `left`/`top`), `willChange: 'contents'` on Swirl canvas, `willChange: 'transform'` + `translateZ(0)` on CaseStudyThumbnail — load-bearing for compositor layering.
...
### The Lab — Session 60, updated Session 61–63, **Session 107**

Live at `/lab`. Open notebook: featured experiments at top, chronological feed below.

**Files:**
- `src/content/lab-experiments.ts` — `ExperimentStatus`, **`ExperimentStep`** (`label`, `done`), **`LabExperiment`** (optional **`steps?: ExperimentStep[]`**) + **`LAB_EXPERIMENTS`** (**6** experiments, **Session 107** full content replace)
- `src/content/lab-feed.ts` — `FeedEntryType` + `FeedEntry` + **`LAB_FEED`** (**11** entries, **Session 107** full content replace)
...
**LabView layout (Session 61, Session 67 mobile, **Session 71** chrome, **Session 107** experiment steps):**
...
- **`useIsMobile()`** — content `padding: isMobile ? '140px 20px 80px' : '160px 48px 160px'`, `maxWidth: 760`, `width: '100%'`, `boxSizing`. `<main overflowX: hidden>`.
- Page order: header → currently thinking about (6 prompts) → divider → things I hold true (7 beliefs) → divider → experiments (**6** cards) → divider → feed
...
- **Session 107 — Steps / progress:** If **`exp.steps?.length`**, render after description **`p`** tags and **before** **`exp.notes`**: 2px track (`rgba(255,255,255,0.06)`), fill **`rgba(0,255,159,0.5)`** at **`doneCount / steps.length`**, checklist with **`✓`** (done, strikethrough label) vs **`○`** (todo, dim), then **`N / M steps complete`**. Omit block when no steps.
...
- Lab long-form copy: **no em dashes** in `lab-experiments` / `lab-feed` strings.
