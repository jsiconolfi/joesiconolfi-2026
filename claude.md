...
- **Session 77 — Streaming + length:** **`max_tokens: 400`** (hard cap; do not raise without explicit instruction). **`SYSTEM_PROMPT` "How to answer"** requires short replies (2-3 sentences simple, 4-5 max detailed; prefer surfacing **cards** over long prose). Server: raw Anthropic body **`pipeThrough`** a **`TransformStream`** with SSE line buffering; response headers **`Cache-Control: no-cache, no-transform`**, **`X-Accel-Buffering: no`**. Cards emitted on **`message_stop`** and **`flush`** (once). Client: **`setMessages` inside the per-line parse loop** for each text chunk (no debounce).
