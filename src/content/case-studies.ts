...
export const CASE_STUDIES: CaseStudy[] = [
  // --- FULL CASE STUDIES ---
  {
    ...
    decisions: [
      ...
      },
      {
        ...
        body: 'Documentation that nobody reads is worthless. I built Sherpa, a Figma plugin that lets designers query the Waypoint documentation in natural language using Cohere\'s embed-english-v3.0 and command-r-plus models with Pinecone as the vector store. The real test was whether designers stopped asking the same questions in Slack. They did.',
        artifact: '/projects/sherpa.mp4',
      },
      {
        title: 'Packaging for external delivery',
        body: 'When RBC became a client needing Waypoint components, I scoped and kicked off the npm packaging work, consolidating icons, components, and tokens into a private package. External delivery has a way of forcing discipline internal use doesn\'t: every undocumented behavior becomes a support burden, every inconsistency becomes a contract violation. The external package is better designed because it had to be.',
      },
    ],
...

  {
    ...
    decisions: [
      {
        ...
        body: 'We defined a skill taxonomy of the discrete components of aiming performance and built every feature on top of it. This gave the product a coherent vocabulary and meant every new feature could be positioned within a framework users already understood. When the AI coaching surfaced a recommendation, it connected it to a specific skill gap rather than just telling users they needed more practice.',
        artifact: '/projects/statespace.mp4',
      },
      {
        title: 'Progressive disclosure of data',
        body: 'New users see simple scores. Advanced users see the full performance model. The same data, different views, controlled by engagement level rather than a settings toggle. Users graduated into complexity naturally, without us having to decide when they were ready. The system inferred readiness from behavior.',
      },
      {
    ...
    outcome: 'Aim Lab reached 30M+ registered users. The training system model became the thing that made it different from every other aim trainer, and the core of the $50M Series B. The Kernel integration was covered by VentureBeat and became the opening move in the cognitive performance category Statespace was building.',
...

  {
    ...
    hook: 'Channel was building AI writing tools at a moment when every AI writing tool looked the same: text box, generate, copy. The problem wasn\'t the underlying model. It was the interaction model. Generate-and-copy treats AI as a vending machine. We were trying to build something you could actually think with.',
    hardPart: 'The hardest design problem was latency. Streaming responses feel broken when the UI isn\'t designed for them. Every interaction pattern had to be rebuilt from first principles for a streaming-first world: how you show thinking, how you show streaming, how you handle errors, how you invite iteration. None of the patterns from static form UX applied. Everything was wrong until we stopped trying to adapt existing patterns and started designing for how the model actually worked.',
    decisions: [
      {
...

  {
    ...
    hook: 'Most brainstorming tools are just note-taking apps with a new name. Seudo started from a different premise: what if the AI was present during the brainstorm itself, not just available after? Voice input removes the friction of typing. Real-time clustering surfaces connections the human might miss while still speaking. The tool thinks alongside you.',
    hardPart: 'Voice input is fast and messy. The AI clustering needed to keep up without overwhelming the user with reorganization while they were still thinking. Getting the timing right required a lot of iteration with real users: when to surface a new cluster, when to let ideas accumulate before organizing them, and how to signal that the system is working without pulling attention away from the thought in progress.',
    decisions: [
      {
        title: 'Voice as the primary input',
        body: 'Typing interrupts thought. Voice is how people actually brainstorm. Building voice as the primary modality (not an add-on) meant rethinking every part of the UI. There\'s no cursor, no text field focus, no submit button. The interface had to feel ambient. The visual layer is a consequence of the voice layer, not the other way around.',
        artifact: '/projects/seudo.mp4',
      },
      {
        title: 'Real-time clustering without interruption',
        body: 'The AI clusters ideas as they arrive but doesn\'t surface reorganizations until natural pauses. This prevents the feeling that the system is constantly moving things around while you\'re still thinking. The clustering is continuous. The UI updates are gated. Users experience the intelligence, not the process.',
      },
      {
    ...
    outcome: 'Users organized ideas 35% faster in testing. The clustering consistently surfaced connections users said they hadn\'t consciously made. The description that came up most often in user sessions was "thinking out loud with someone who\'s actually listening." Still building.',
...

  {
    ...
    hook: 'Wafer is reimagining the smartphone OS from the ground up for an AI-native world. The premise: apps are the wrong abstraction. Instead of siloed apps that don\'t talk to each other, Wafer builds a unified context layer that understands what you\'re doing across everything and surfaces the right capability at the right moment. The question is not "which app do I open?" It is "what do I need right now?"',
    hardPart: 'Designing for an OS that doesn\'t exist yet, for hardware that isn\'t shipping yet, for users who don\'t know they want it yet. The design had to be specific enough to be buildable and vague enough not to be wrong. The practical constraint: every decision I made would be validated or invalidated by hardware that didn\'t exist. So the design had to be principled, not prescriptive. Build the patterns, not the pixels.',
    decisions: [
      {
        title: 'Context layer as the core metaphor',
        body: 'Instead of designing apps, we designed the context layer: the surface that sits above apps and understands user intent. This meant designing states, transitions, and affordances for a system that\'s always present but rarely front-and-center. The design challenge is not what it looks like when you\'re using it. It\'s what it looks like when it interrupts you at exactly the right moment.',
        artifact: '/projects/wafer.mp4',
      },
      {
        title: 'Proactive vs reactive UI',
        body: 'Traditional mobile UI is reactive. You open an app, you do a thing. Wafer\'s UI is proactive. The system anticipates and surfaces. Designing for proactivity required rethinking every interaction model. The question stopped being "what does the user do next?" and became "what does the user need before they know they need it?" That\'s the context inference problem operating at the OS level.',
      },
      {
        title: 'Component library for an AI OS',
        body: 'An AI-native OS surfaces in wildly different contexts: a cooking timer, a meeting note, a driving direction. The components had to feel like they belonged to the same coherent system regardless of where they appeared. I built 12 categories of components with a unified visual language specifically for that constraint. The goal was: users should feel the system before they understand it.',
      },
    ],
    outcome: 'A full component system and interaction model for Wafer\'s context layer. The OS is in active development. The design work continues as the hardware constraints become clearer and the scope of what the context layer can do expands.',
...

  {
    ...
    hook: 'A design system is only as good as its adoption. The bottleneck at Cohere wasn\'t the components. It was that designers couldn\'t find what they needed fast enough. They\'d open Figma, try to remember the right component name, give up, and build something custom. A Figma plugin that answered questions about Waypoint in natural language removed the friction between having a system and actually using it.',
    hardPart: 'RAG quality is highly sensitive to how you chunk and embed documentation. Early versions surfaced technically correct but contextually wrong answers. The right component for the wrong use case, basically. Getting retrieval quality right required rethinking the documentation structure itself, not just the pipeline. If the documentation was vague, the answers were vague. The AI was a mirror of the documentation quality.',
    decisions: [
      {
      ...
      },
      {
        ...
        body: 'Responses aren\'t just retrieved text. They\'re generated answers grounded in the retrieved chunks. command-r-plus synthesizes the relevant documentation into a direct answer with component names, variant recommendations, and links to the relevant Figma frames. The difference between search and Sherpa is that search returns a document. Sherpa returns an answer.',
      },
      {
        title: 'Figma-native UX',
        body: 'The plugin had to feel like it belonged in Figma, not like a chat window dropped in from somewhere else. The interaction model mirrors how designers already talk about decisions: "what should I use for this?" rather than searching for component names. The context is Figma. The language should be too. A plugin that forces designers to think in system vocabulary defeats the point.',
      },
    ],
  ...
  // --- QUICK TAKES ---
  {
    ...
    hook: 'Design tokens drift between Figma and code. It always happens. The designer updates a color, the engineer has a different version, the gap widens until someone manually reconciles it. waypoint-sync is a CLI tool that makes the design file the source of truth the codebase reads from.',
    decisions: [
      {
        title: 'design-map.json as the source of truth',
        body: 'A single JSON file maps every Figma variable to its CSS and Tailwind counterpart. The CLI reads this map and either pushes code changes to Figma or pulls Figma changes to code. One source of truth, two directions. The design file stops being a spec and starts being an API.',
        artifact: '/projects/waypoint-sync.mp4',
      },
      {
        title: 'Claude Code + Cursor as the runtime',
        body: 'Rather than building a custom sync engine, I use Claude Code and Cursor as the execution layer. The design-map.json is the spec, and the AI handles the actual file operations. This keeps the tool lightweight and maintainable. The interesting finding: you don\'t need custom tooling if the spec is precise enough for the model to execute against.',
      },
    ],
...

  {
    ...
    hook: 'Kernel Flow is a neural interface that measures cognitive load and focus in real time. Integrating it with Aim Lab meant designing a UX that made neuroscience data legible and actionable for gamers, not researchers. The data was sophisticated. The interface had to be the opposite.',
    decisions: [
      {
      ...
      },
      {
        ...
        body: 'The neural data needed to enhance training sessions without adding friction. We designed it as an ambient layer. Visible when you want it. Invisible when you\'re in the zone. The worst version of this feature would be one that made you think about your brain while you were trying to train.',
      },
    ],
    outcome: 'The integration shipped and VentureBeat covered it. The cognitive performance angle became a real differentiator for Statespace and part of the $50M Series B story.',
...

  {
    ...
    hook: 'Mushroom was building LLM-powered voice interfaces before voice AI was mainstream. The design challenge was fundamental: there\'s no screen to design. No cursor, no button, no text field. The interface is entirely audio, and the feedback is entirely acoustic.',
    decisions: [
      {
        title: 'Designing for ears, not eyes',
        body: 'Every interface decision was evaluated aurally first. Response length, pacing, confirmation patterns: all optimized for how they sound, not how they look. This required new design tools and new evaluation methods. The standard UX toolkit doesn\'t exist for voice. We built our own.',
        artifact: '/projects/mushroom.jpg',
      },
      {
        title: 'Conversational state management',
        body: 'Voice conversations have context that text doesn\'t: tone, pace, interruptions. We designed a state model that accounted for these signals and let the interface adapt to the emotional and practical context of each interaction. A user who sounds frustrated gets a different response pattern than a user who sounds engaged.',
      },
    ],
    outcome: 'Shipped production voice interfaces across multiple products. The biggest finding was about latency: users\' threshold for feeling understood is much lower with voice than text. A 200ms response feels intelligent. A two-second response feels broken, regardless of what it says.',
    ...
    decisions: [
      {
        ...
        body: 'Not all prototypes need to be high fidelity. I build at the fidelity the question requires: sometimes a rough interactive mock to test if the capability is even understandable, sometimes a near-production component to test if the implementation is right. Building at the wrong fidelity wastes time and produces misleading signals.',
        artifact: '/projects/cohere-labs.mp4',
      },
      {
        title: 'UX as a research input',
        body: 'The prototypes feed back into model development. How users interact with a capability surfaces edge cases and failure modes that pure model evaluation misses. Design and research are a closed loop. The prototype is not just a deliverable. It is a measurement instrument.',
      },
    ],
    outcome: 'Several prototypes have shipped to production or directly shaped what shipped. The more valuable output is often the failure cases: understanding what users cannot make sense of at all helps the research team understand what to improve in the model before it reaches product.',
