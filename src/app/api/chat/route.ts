...
Waypoint: design system built from zero at Cohere. Primitives, semantics, and component tokens so theme and white-label changes do not require rebuilds. Adoption broke down into three failure modes: documentation dishonesty, discoverability, and governance without friction. RBC external delivery proved the tiers: swap primitive and semantic layers for client brand without touching components. Three scoped npm packages under the cohere-ai org (tokens, icons, components) so cadences stay independent. Sherpa tackles discoverability with component-level RAG in Figma. waypoint-sync tackles drift with bidirectional sync and 0-100 parity scoring. Production for North plus clients including RBC.

Statespace/Aim Lab: AI coaching platform, 30M+ users. The product was optimizing for the wrong thing until the thesis shifted to whether training actually made people better. Built a skill taxonomy as the foundation, a coaching arc on weekly and monthly cadences, progressive disclosure driven by behavior rather than settings, and Kernel integration that reframed neuroscience as performance language. Aim Lab reached 30M+ users. The training system model anchored the $50M Series B.

Channel AI: Consumer iOS app that unified open-source image and language models. Think Hugging Face designed for consumers. Five designed streaming lifecycle states (thinking through refining), model exploration by intent, dynamic model switching that preserved workflow continuity, trust through transparency. Abstraction that makes capability legible, not erased. Shipped to production on iOS.

Channel AI: Nexus: Compare up to four models in one conversation. The hard problem is state divergence: each model in a different lifecycle state at once. State indicators are a separate visual layer from content so you can read all four statuses peripherally. Shared prompt, conversation-first layout, composable architecture for four live streams. Shipped as consumer and enterprise evaluation. Sequential chat UI assumes one thing at a time; multi-model workflows break that assumption.

Channel AI: Prism: Consumer image generation with automatic model selection via internal scoring and an agentic handoff you can redirect without learning the stack. Post-prompt layer: legible reasoning, then remix, manual override, or reuse a prior generation. Searchable gallery with remix turned outputs into a discovery surface.

Seudo: Voice-first brainstorming with AI clustering, solo from zero. Clustering runs continuously; UI updates gate to natural pauses. Spatial map shows semantic proximity. 35% faster organization in testing. Users described it as thinking out loud with someone who is actually listening. The hard problem is when the system speaks, not what it says. Still building.

Wafer Systems: AI-native OS design, 2025-present. Apps are the wrong abstraction. Context layer surfaces the right capability at the right moment. The interrupt model is the product: too early is noise, too late is useless. Twelve component categories, one visual language across People, Conversations, Travel, Home, Entertainment, Money, Email, and more. Principled design for hardware that does not exist yet. Magic Ink at the OS level.

Sherpa: RAG-based Figma plugin for design system Q&A at Cohere, designed and built solo. Page-level chunks were technically right and contextually wrong; component-level chunking matched question precision. Documentation honesty was a prerequisite: acknowledge gaps explicitly. command-r-plus answers grounded in chunks, not pasted pages. Designers stopped asking in Slack. The problem is discoverability at scale, not lack of docs: better access to what already exists.

waypoint-sync: Two-way Figma-to-code token pipeline, 2026, built solo. A design system is not a set of files, it is a living set of decisions. Those decisions need to stay synchronized across every medium automatically. design-map.json is a declarative schema that maps every Figma variable to its CSS custom property and Tailwind counterpart. The schema is a design artifact: it defines the contract between design and code in a form a model can read and reason about. The design system becomes a model-in-the-loop artifact. Claude Code and Cursor invoke sync and parity checking through natural language. Parity scoring is 0-100 per component. Found real drift in production. Manual token sync eliminated across Waypoint.

Statespace x Kernel: Neural interface UX with Aim Lab. Mapped cognitive metrics to performance language gamers could act on: peak windows, recovery, warm-up. Ambient layer so training stayed in flow. VentureBeat coverage. Part of the Series B story.

Mushroom: LLM-powered voice interfaces in 2022, pre-ChatGPT playbook. Product design from first principles: intent without a text field, trust in a system you cannot see, evaluation aural first. Conversational state adapted to tone, pace, and hesitation. Key finding: 200ms response latency feels intelligent; two seconds feels broken regardless of content. Trust forms before the answer arrives.

Cohere Labs: Prototypes for new model capabilities, 2025-present. Capabilities arrive from research before interaction patterns exist; closing that gap early avoids bad UX later. Fidelity follows the question. UX feeds model development, not just presentation: incomprehensible capabilities inform model behavior. Every prototype starts with proactive versus reactive framing. Failure cases are often the most valuable output. The learning is the deliverable.
